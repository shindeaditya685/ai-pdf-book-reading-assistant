...
export default function QuoteChatDetailPage() {
      ...
      if (res.ok) {
        ...
        setPinnedQuotes((prev) => {
          ...
          const hydrated: HydratedQuote = {
            ...
            color: target.color || 'rgba(16, 185, 129, 0.5)',
          }
  ...
  if (error) {
    return (
      <div className="flex h-screen flex-col items-center justify-center gap-3 bg-canvas p-6 text-center">
        <AlertCircle className="h-10 w-10 text-red-500" />
        <p className="font-serif text-sm font-bold tracking-tight text-ink">{error}</p>
        <div className="flex gap-2">
          <button
            onClick={() => load()}
            className="rounded-lg bg-brand px-3 py-1.5 text-xs font-semibold text-brand-fg shadow-sm transition-all hover:brightness-110 active:scale-[0.97]"
          >
            Retry
          </button>
          <Link href="/quotes/chat" className="rounded-lg border border-paper-border px-3 py-1.5 text-xs font-semibold text-muted-foreground hover:bg-muted/40 transition-colors">
            Back
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="flex h-screen flex-col bg-canvas text-ink">
      {/* ── HEADER ── */}
      <header className="flex h-14 shrink-0 items-center justify-between border-b border-paper-border bg-canvas/85 px-3 backdrop-blur-xl sm:px-4">
        <div className="flex min-w-0 items-center gap-2 sm:gap-3">
          <Link href="/quotes/chat" className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg border border-paper-border text-muted-foreground transition-all hover:border-ink/20 hover:bg-muted/40 hover:text-ink">
            ...
            {editingTitle ? (
              <div className="flex items-center gap-1.5">
                <input
                  ...
                  autoFocus
                  className="h-7 max-w-[260px] flex-1 rounded-md border border-paper-border bg-muted/40 px-2 text-sm font-semibold text-ink outline-none transition-all focus:border-brand/50 focus:ring-1 focus:ring-brand/15 sm:max-w-md"
                />
                <button
                  onClick={handleSaveTitle}
                  className="flex h-7 w-7 items-center justify-center rounded-md text-emerald-600 hover:bg-emerald-500/10 dark:hover:bg-emerald-500/15"
                  title="Save title"
                >
              ...
              >
                <span className="truncate font-serif text-base font-bold tracking-tight text-ink">{conversation.title}</span>
        ...
        <div className="flex shrink-0 items-center gap-1">
          <button
            ...
            className="hidden h-8 items-center gap-1.5 rounded-lg border border-paper-border bg-card px-2.5 text-xs font-semibold text-muted-foreground transition-colors hover:text-ink sm:inline-flex"
      ...
      <div className="flex min-h-0 flex-1">
        <div className="flex min-w-0 flex-1 flex-col">
          ...
          <div className="flex-1 overflow-y-auto px-3 py-4 sm:px-6">
            {messages.length === 0 ? (
              <div className="mx-auto flex max-w-2xl flex-col items-center justify-center gap-3 py-16 text-center">
                ...
                <h2 className="font-serif text-lg font-bold tracking-tight text-ink">Ask about your quotes</h2>
                ...
                {pinnedQuotes.length === 0 && (
                  <button
                    ...
                    className="mt-2 inline-flex h-9 items-center gap-1.5 rounded-lg bg-brand px-3 text-xs font-semibold text-brand-fg shadow-sm transition-all hover:brightness-110 active:scale-[0.97]"
                  >
                    <QuoteIcon className="h-3.5 w-3.5" />
                    Pin some quotes
                  </button>
              ...
              <div className="mx-auto max-w-2xl space-y-4">
                ...
                {sending && (
                  <div className="flex items-start gap-2.5">
                    ...
                    <div className="flex-1 rounded-2xl rounded-tl-sm bg-muted/40 px-3.5 py-2.5">
          ...
          {/* Composer */}
          <div className="shrink-0 border-t border-paper-border/60 bg-canvas/85 px-3 py-3 backdrop-blur-xl sm:px-6">
            <div className="mx-auto max-w-2xl">
              {pinnedQuotes.length > 0 && (
                <div className="mb-2 flex flex-wrap items-center gap-1.5">
                  ...
                  {pinnedQuotes.slice(0, 4).map((q) => (
                    <span
                      key={q.id}
                      className="inline-flex max-w-[160px] items-center gap-1 rounded-full bg-brand/10 px-2 py-0.5 text-[10px] font-semibold text-brand"
                      title={q.text}
                    >
              ...
              <div className="flex items-end gap-2 rounded-2xl border border-paper-border bg-card p-2 shadow-sm focus-within:border-brand/50 focus-within:ring-1 focus-within:ring-brand/30">
                <textarea
                  ref={inputRef}
                  value={draft}
                  ...
                  className="max-h-40 min-h-[36px] flex-1 resize-none bg-transparent px-2 py-1.5 text-sm text-ink placeholder:text-muted-foreground/60 focus:outline-none disabled:opacity-50"
                />
                <button
                  onClick={handleSend}
                  disabled={sending || !draft.trim() || pinnedQuotes.length === 0}
                  className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-brand text-brand-fg shadow-sm transition-all hover:brightness-110 active:scale-[0.97] disabled:opacity-50"
                  title="Send (Enter)"
        ...
        {showPinPanel && (
          <aside className="hidden w-80 shrink-0 border-l border-paper-border/60 bg-canvas/60 backdrop-blur-md lg:flex lg:flex-col">
            <div className="border-b border-paper-border/60 px-4 py-3">
              <div className="flex items-center justify-between">
                <h3 className="font-serif text-sm font-bold tracking-tight text-ink">Pinned Quotes</h3>
                <button
                      ...
                      <button
                        ...
                        className={`group block w-full rounded-lg border p-2.5 text-left transition-colors ${
                          isPinned
                            ...
                            : 'border-paper-border bg-muted/20 hover:border-brand/40'
        ...
        {showPinPanel && (
          <div className="fixed inset-x-0 bottom-0 z-40 max-h-[70vh] overflow-hidden rounded-t-2xl border-t border-paper-border/60 bg-canvas/95 shadow-2xl backdrop-blur-md lg:hidden">
            <div className="flex items-center justify-between border-b border-paper-border/60 px-4 py-3">
              <h3 className="font-serif text-sm font-bold tracking-tight text-ink">Pinned Quotes</h3>
              <button
                onClick={() => setShowPinPanel(false)}
                className="flex h-7 w-7 items-center justify-center rounded-md text-muted-foreground hover:bg-muted/40"
                    ...
                    return (
                      <button
                        ...
                        className={`block w-full rounded-lg border p-2.5 text-left transition-colors ${
                          isPinned ? 'border-brand/50 bg-brand/10' : 'border-paper-border bg-muted/20'
                        } ${overLimit ? 'cursor-not-allowed opacity-50' : ''}`}
                      >
                        <p className="line-clamp-3 text-xs leading-relaxed text-ink">
                          &ldquo;{q.text}&rdquo;
                        ...
                        <div className="mt-1.5 flex items-center justify-between text-[9px] text-muted-foreground/50">
                          <span className="truncate">{q.pdfFileName.split('/').pop()} · p.{q.pageNumber}</span>
                          <span className="font-bold">
                            {isPinned ? <span className="text-brand">Pinned</span> : overLimit ? 'Limit' : '+ Pin'}
                          </span>
                        </div>
                      </button>
                    )
      ...
      {quotaModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
          <div className="w-full max-w-sm rounded-xl border border-paper-border bg-card p-6 shadow-2xl">
            <h3 className="font-serif text-sm font-bold tracking-tight text-ink">AI limit reached</h3>
  ...
  return (
    <div className={`flex items-start gap-2.5 ${isUser ? 'flex-row-reverse' : ''}`}>
      ...
      <div className={`flex max-w-[85%] flex-col gap-1.5 ${isUser ? 'items-end' : 'items-start'}`}>
        ...
        <div
          className={`rounded-2xl px-3.5 py-2.5 text-sm leading-relaxed shadow-sm ${
            isUser
              ...
              : 'rounded-tl-sm bg-muted/50 text-ink'
          }`}
        >
          {message.content ? (
        ...
        {refs.length > 0 && (
          <details className="group w-full max-w-md rounded-lg border border-paper-border bg-muted/20 px-2.5 py-1.5 text-xs">
            <summary className="cursor-pointer text-[10px] font-semibold uppercase tracking-wider text-muted-foreground/60 hover:text-ink">
              {refs.length} referenced quote{refs.length === 1 ? '' : 's'}
            </summary>
            <div className="mt-2 space-y-2">
              {refs.map((r) => (
                <div key={r.quoteId} className="rounded-md border border-paper-border/60 bg-muted/30 p-2">
                  <p className="line-clamp-3 italic text-ink">&ldquo;{r.text}&rdquo;</p>
